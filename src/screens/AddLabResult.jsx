    ...
    handleFormSubmit(e) {
        ...
        Appointment.addLabResult(dataJson).then(response => {
            if (response.status === 200) {
                ...
                alert(`Sukses add hasil lab pasien ${this.state.pasien.nama} dengan hasil ${response.result.hasil}`)
            } else {
                this.setState({
                    loading: false
                })
                alert(`Gagal add hasil lab pasien bernama ${this.state.pasien.nama}`)
            }
        })
    }
